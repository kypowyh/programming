...
cJSON* readJsonFromFile(const char* filename) {
    ...
    if (fileSize == 0) {
        fclose(file);
        return cJSON_CreateArray();  // Если файл пуст, вернуть новый пустой массив
    }
...
void JADD(const char* ip, const char* url, const char* time) {
    // Чтение существующего JSON из файла
    cJSON* jsonArray = readJsonFromFile(FILENAME);
    ...
    // Создание нового объекта JSON и добавление его в массив
    cJSON* newObj = cJSON_CreateObject();
    cJSON_AddStringToObject(newObj, "ip", ip);
    cJSON_AddStringToObject(newObj, "time", time);
    cJSON_AddStringToObject(newObj, "url", url);

    if (cJSON_IsArray(jsonArray)) {
        cJSON_AddItemToArray(jsonArray, newObj);
    }
    else {
        // Если jsonArray не является массивом, создать новый массив
        cJSON* newArray = cJSON_CreateArray();
        cJSON_AddItemToArray(newArray, newObj);
    ...
    if (fopen_s(&file, FILENAME, "w") != 0) {
        ...
        cJSON_Delete(jsonArray);  // Освободить ресурсы, если что-то пошло не так
        return;
    }
...
char* processQuery(struct HashTable* ht, struct Set* mySet, struct Stack* myStack, struct Queue* myQueue, const char* query) {
    ...
    else if (strcmp(command, "JADD") == 0) {
        // Получаем ip, url и time из строки
        char* ip = strtok_s(NULL, " ", &context);
        char* url = strtok_s(NULL, " ", &context);
        char* time = strtok_s(NULL, " ", &context);

        // Проверяем, что все значения были получены
        if (ip != NULL && url != NULL && time != NULL) {
            // Вызываем processAndAppendJson
            JADD(ip, url, time);

            result = _strdup("JADD command processed.");
        }
        else {
            result = _strdup("Invalid JADD format.");
        }
    }
    else if (strcmp(command, "JGET") == 0) {
        // Вызываем функцию 
        char* jsonString = JGET();

        if (jsonString != NULL) {
            result = jsonString;
        }
        else {
            result = _strdup("Failed to retrieve JSON string.");
        }
    }
    else {
...
DWORD WINAPI Client(LPVOID Param) { // функция обрабатывается в новом потоке
    ...
    char* responseBuffer = (char*)malloc(1024); // Выделяем память для строки запроса 

    int erStat = recv(ClientConn, responseBuffer, 1024, 0); // принимаем строку запроса 
