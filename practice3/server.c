...
CRITICAL_SECTION fileMutex; // мьютекс для файла
...
void writeDataToFile(const char* filename, struct HashTable* ht, struct HashSet* mySet, struct Stack* myStack, struct Queue* myQueue) {
    EnterCriticalSection(&fileMutex); // открываем критическую секцию
    ...
    LeaveCriticalSection(&fileMutex); // закрываем критическую секцию 
}
